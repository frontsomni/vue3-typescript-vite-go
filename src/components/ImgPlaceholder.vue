<template>
  <div :class="isLoadImgSucc ? 'load-img-succ' : 'load-img-error'" :alt="alt" :style="{
    width,
    height
  }">
    <img class="img-placeholder" :src="src" @error.once="onLoadImgError" />
  </div>
</template>
...
interface IImgPlaceholder {
  onLoadImgError(): void
  isLoadImgSucc: boolean
}
...
export default defineComponent({
  ...
  setup() {
    let isLoadImgSucc = ref<IImgPlaceholder['isLoadImgSucc']>(true)
    const onLoadImgError: IImgPlaceholder['onLoadImgError'] = () => {
      isLoadImgSucc.value = false
    }
    return {
      onLoadImgError,
      isLoadImgSucc
